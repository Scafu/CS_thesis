...
), e da _Apple_ per _iOS_ (_Human Interface Guidelines_) #cite(<ios-accessibility>), integrate con le funzionalità di
accessibilità native del _framework_ _Flutter_. _Flutter_ gestisce l'accessibilità attraverso un albero semantico che
vive parallelamente all'albero dei _widget_. Questo albero viene utilizzato dai servizi di assistenza delle rispettive
piattaforme, _TalkBack_ per _Android_ e _VoiceOver_ per _iOS_, che forniscono feedback vocali e altre forme di
assistenza. Ogni componente interattivo dell'applicazione espone etichette semantiche e, ove applicabile, un _flag_
`isSelected` per indicare lo stato di selezione. Ciò riguarda in particolare i componenti di navigazione, come la
`CustomNavigationBar`, e i controlli di selezione come _toggle_, la cui corretta semantica è stata verificata attraverso
...
`iOSTapTargetGuideline` e `labeledTapTargetGuideline`, #cite(<flutter-accessibility>)) che viene eseguita sui componenti
...
I test che verrano che verranno descritti di seguito sono stati eseguiti grazie ad una libreria nativa di _Flutter_,
...
I test di integrazione verificano il corretto funzionamento di più unità logiche, questi test vengono eseguiti in un
ambiente di _Dart_ puro. Nel contesto del progetto questi test verificano che _datasource_ e _repository_ si integrino
correttamente con le fonti di dati esterni, garantendo che l'applicazione funzioni anche senza la connessione di rete.
Questi test sono identificati da questa convenzione:
#v(0.5em)
#align(center, text(12pt, weight: "semibold", "T[Tipo][Numero]"))
#v(0.5em)
dove:
...
== Test di Sistema
I test di sistema verificano il corretto funzionamento dell'applicazione nel suo complesso, eseguendo simulazioni
direttamente sul dispositivo fisico o sull'emulatore. A differenza dei test di unità e di integrazione, questi test
osservano l'applicazione come l'utente finale, senza conoscere la struttura interna del codice. Questi test usano un
altro pacchetto _Flutter_, chiamato _Integration_test_, che permettono di eseguire test e simulano azioni direttamente
sul dispositivo. Le verifiche si concentrano sulla risposta visiva dell'interfaccia (presenza di _widget_, assenza di
...
Alla fine di ogni _sprint_, veniva organizzato un incontro di revisione con il _tutor_ interno al fine di conoscere lo
stato di avanzamento del progetto, discutere eventuali criticità riscontrate e pianificare le attività per lo sprint
successivo. Durante questi incontri, venivano presentati i risultati raggiunti, inclusi i _test_ implementati e
superati, e si discuteva delle funzionalità da dover sviluppare. Inoltre, veniva effettuata una dimostrazione pratica
dell'applicazione, mostrando le funzionalità implementate e il loro funzionamento. Questi incontri di revisione erano
fondamentali per garantire che lo sviluppo procedesse in linea con le aspettative e per ricevere feedback tempestivi,
che permettevano di apportare eventuali correzioni o miglioramenti al progetto in modo efficace.

L'ultima settimana di stage è stata dedicata alla fase del rilascio e agli eventuali _hotfix_ necessari. Durante questa
fase, il prodotto è stato consegnato al _tutor_ interno e al proponente, e sono stati raccolti i _feedback_ finali.

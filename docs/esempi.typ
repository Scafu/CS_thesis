#import "../config/thesis-config.typ": gl, glpl, linkfn
...
== Bibliografia
Per la gestione della bibliografia leggi il file `appendix/bibliography/bibliography.typ`. Le citazioni si fanno con \@
es. @models-and-fair-use.

== Blocchi di codice
È possibile inserire blocchi di codice usando la libreria #linkfn("https://typst.app/universe/package/codly/")[Codly]
(dopo averla importata). Il logo e i colori del testo sono gestiti dalla libreria Codly-languages. I blocchi di codice
vanno sempre dentro una figure così da inserirli nell'indice e poterli riferire nel testo, ad esempio:
@fig:codice-esempio. #figure(
...
È possibile aggiungere uno script che compila il codice ad ogni push (`build`) e pubblica il PDF su GitHub pages
(`deploy`). Questo script va inserito in `.github/workflows/main.yaml`\
Nota bene: se scegli di usare dei font diversi da quelli inclusi in Typst o installati nel runner verrà usato un font di
default.
#figure(
  caption: [GitHub CI],
  ```yaml
...
== Funzione `linkfn`
Questa funzione mi è tornata utile durante il capitolo di studio delle tecnologie. Mette il link sia sul testo che nella
footnote in modo da rendere il testo "accessibile" anche per la stampa.\
Esempi:\
#v(1em)
...
#v(1em)
Nota: In Typst ci sono più modi di invocare una funzione che accetta del `content` (trailing content). #link(
  "https://typst.app/docs/reference/foundations/function/",
)

== Grafici vari
Con le librerie del #linkfn("https://typst.app/universe")[Typst Universe] è possibile creare grafici avanzati. Ad
esempio utilizzando #linkfn("https://typst.app/universe/package/chronos")[Chronos] ho creato il diagramma in
@fig:diagramma-git. #figure(
  chronos.diagram({
    import chronos: *
...
Se devi lavorare con i diagrammi degli use case direttamente in Typst è fattibile ma un po' complicato. Fai riferimento
alla documentazione di questo progetto di SWE:
- #link("https://techminds-unipd.github.io/docs/PB/documenti_esterni/analisi_requisiti/analisi-dei-requisiti.pdf")
- #link(
    "https://github.com/techminds-unipd/docs/blob/main/PB/documenti_esterni/analisi_requisiti/include/use_cases.typ",
  )
#v(1em)
Il tracciamento dei requisiti viene fatto attraverso la tabella in `/chapters/data/requirements_list.typ` Questo
permette di:
